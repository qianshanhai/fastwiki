...
int WikiHistory::wh_init()
{
	...
	m_hash->sh_get_user_data((void **)&m_head);
...
int WikiHistory::wh_set_last_record(const history_key_t *key, const history_value_t *value)
{
	memcpy(&m_head->last_key, key, sizeof(m_head->last_key));
	memcpy(&m_head->last_value, value, sizeof(m_head->last_value));

	return 0;
}

int WikiHistory::wh_get_last_record(history_key_t *last_key, history_value_t *last_value)
{
	memcpy(last_key, &m_head->last_key, sizeof(*last_key));
